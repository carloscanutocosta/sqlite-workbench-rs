...
impl Default for ColumnRow {
    fn default() -> Self {
        Self { name: String::new(), col_type: "TEXT".into(), pk: false, ai: false, nn: false }
    }
}
...
impl CreateTableDialog {
    pub fn new() -> Self {
        ...
        d.columns.push(ColumnRow { name: "id".into(), col_type: "INTEGER".into(), pk: true, ai: true, nn: false });
        d
    }

    /// Returns Some((name, cols, fks)) when user clicks Save.
    pub fn show(&mut self, ui: &mut Ui, t: &'static T, existing_tables: &[String]) -> Option<(String, Vec<ColumnDef>, Vec<ForeignKeyDef>)> {
        let mut result = None;
        ...
        // Header
        ui.horizontal(|ui| {
            ui.add_sized([150.0, 18.0], egui::Label::new(RichText::new(t.col_name).strong()));
            ui.add_sized([90.0, 18.0], egui::Label::new(RichText::new(t.col_type).strong()));
        ...
        ScrollArea::vertical().id_salt("col_rows").max_height(180.0).show(ui, |ui| {
            ...
            for (i, row) in self.columns.iter_mut().enumerate() {
                ui.horizontal(|ui| {
                    ...
                    if ui.small_button(RichText::new(t.remove).color(Color32::RED)).clicked() {
                        to_remove = Some(i);
                    }
                });
            }
            if let Some(i) = to_remove { self.columns.remove(i); }
        });
        ...
        ScrollArea::vertical().id_salt("fk_rows").max_height(120.0).show(ui, |ui| {
            let mut to_remove = None;
            for (i, fk) in self.fks.iter_mut().enumerate() {
                ui.horizontal(|ui| {
                    ui.add_sized([120.0, 20.0], egui::TextEdit::singleline(&mut fk.from_col).hint_text(t.col_name));

                    let ref_table_display = if fk.ref_table.is_empty() { "—".to_string() } else { fk.ref_table.clone() };
                    ...
                    ui.add_sized([100.0, 20.0], egui::TextEdit::singleline(&mut fk.ref_col).hint_text("id"));
                    if ui.small_button(RichText::new(t.remove).color(Color32::RED)).clicked() {
                        to_remove = Some(i);
                    }
                });
            }
            if let Some(i) = to_remove { self.fks.remove(i); }
        });

        if ui.button(t.add_fk).clicked() {
            self.fks.push(FkRow { from_col: String::new(), ref_table: String::new(), ref_col: "id".into() });
        }

        ui.separator();
        ui.horizontal(|ui| {
            if ui.button(t.save).clicked() && !self.table_name.trim().is_empty() {
                let cols: Vec<ColumnDef> = self.columns.iter().map(|r| ColumnDef {
                    name: r.name.clone(),
                    col_type: r.col_type.clone(),
                    pk: r.pk, ai: r.ai, nn: r.nn,
                }).collect();
                let fks: Vec<ForeignKeyDef> = self.fks.iter()
...
impl EditRecordDialog {
    pub fn for_insert(columns: Vec<String>) -> Self {
        ...
        Self { rowid: None, columns, values }
    }

    pub fn for_edit(rowid: i64, columns: Vec<String>, values: Vec<String>) -> Self {
        Self { rowid: Some(rowid), columns, values }
    }

    /// Returns Some(pairs) when user clicks Save/Insert.
    pub fn show(&mut self, ui: &mut Ui, t: &'static T) -> Option<Vec<(String, String)>> {
        let mut result = None;

        ScrollArea::vertical().id_salt("edit_scroll").max_height(400.0).show(ui, |ui| {
        ...
        ui.horizontal(|ui| {
            if ui.button(t.cancel).clicked() {
                // Signals nothing — caller handles via window open state
            }
            let btn_label = if self.rowid.is_some() { t.save } else { t.insert };
            if ui.button(btn_label).clicked() {
                let pairs: Vec<(String, String)> = self.columns.iter().cloned()
...
impl ErdWindow {
    pub fn new(tables_data: Vec<(String, Vec<String>, Vec<ForeignKey>)>) -> Self {
        ...
        let tables: Vec<ErdTable> = tables_data.into_iter().enumerate().map(|(i, (name, cols, fks))| {
            let h = 30.0 + cols.len() as f32 * 20.0;
            let w = 160.0f32;
            let angle = std::f32::consts::TAU * i as f32 / n as f32;
            let x = center_x + radius * angle.cos() - w / 2.0;
            let y = center_y + radius * angle.sin() - h / 2.0;
            ErdTable { name, cols, fks, x, y, w, h }
        }).collect();

        Self { tables, drag_idx: None, drag_offset: egui::Vec2::ZERO }
    }
    ...
    pub fn show(&mut self, ui: &mut Ui) {
        ...
        // Draw FK lines
        let positions: Vec<(f32, f32)> = self.tables.iter()
        ...
        for (i, t) in self.tables.iter().enumerate() {
            for fk in &t.fks {
                if let Some(j) = self.tables.iter().position(|t2| t2.name == fk.ref_table) {
                    let p1 = egui::Pos2::new(rect.left() + positions[i].0, rect.top() + positions[i].1);
                    let p2 = egui::Pos2::new(rect.left() + positions[j].0, rect.top() + positions[j].1);
                    painter.line_segment([p1, p2], egui::Stroke::new(1.5, Color32::from_rgb(80, 80, 120)));
                    // Arrow head
        ...
        for (i, t) in self.tables.iter().enumerate() {
            ...
            painter.rect_stroke(box_rect, 4.0, egui::Stroke::new(1.0, Color32::from_rgb(80, 80, 90)));
